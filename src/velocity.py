...
def velocity(psi_grid, nodes_num, nodes_dom, h):
    ...
    horiz_speeds = np.zeros_like(psi_grid, dtype=np.longdouble)
    vert_speeds = np.zeros_like(psi_grid, dtype = np.longdouble)
    norm_speeds = np.zeros_like(psi_grid, dtype = np.longdouble)
