...
def createSystem(path, num, dom, cl = None):
    """
    ...
    nodes_num = np.loadtxt(path + '/' + num, dtype = int)
    nodes_dom = np.loadtxt(path + '/' + dom, dtype = int)
    if cl != None:
        nodes_cl = np.loadtxt(path + '/' + cl, dtype = float)
...
if __name__=="__main__":
    # Creating the system
    A, b = createSystem(".", "1-num.txt", "1-dom.txt", "1-cl.txt")
